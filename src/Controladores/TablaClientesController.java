...
//        abre la ventana de agregar actividades
        if (e.getSource() == busqueda.btnagregaractividad) {
            int selecionar = busqueda.tabladatos.getSelectedRow();
            if (selecionar != -1) {
            AgregarActividad agactividad = new AgregarActividad(busqueda, true);
            Actividad macciones = new Actividad();
            macciones.setCodigo(String.valueOf(busqueda.tabladatos.getValueAt(selecionar, 5)));
            macciones.setEmpresa(String.valueOf(busqueda.tabladatos.getValueAt(selecionar, 4)));
            AgregarActividadesController cag = new AgregarActividadesController(macciones, agactividad, mod);
            try {
                cag.iniciar();
            } catch (IOException ex) {
